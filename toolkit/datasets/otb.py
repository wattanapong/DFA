...
from .video_oneshot import VideoOneShot
...
class OTBOneShotVideo(VideoOneShot):
    """
    Args:
        name: video name
        root: dataset root
    ...
    """
    def __init__(self, name, root, video_dir, init_rect, img_names,
            gt_rect, attr, config=None, load_img=False):
        super(OTBOneShotVideo, self).__init__(name, root, video_dir,
                init_rect, img_names, gt_rect, attr, load_img, config=config)
    ...
    def load_tracker(self, path, tracker_names=None, store=True):
        ...
        for name in tracker_names:
            ...
            if not os.path.exists(traj_file):
                ...
                traj_file = os.path.join(path, name, _name + '.txt')
            ...
            if os.path.exists(traj_file):
                with open(traj_file, 'r') as f :
                    ...
                    if ',' in texts[0]:
                        ...
                        pred_traj = [list(map(float, x.split()))
                                     for x in texts]
...
class OTBDataset(Dataset):
    ...
    """
    def __init__(self, name, dataset_root, load_img=False, config=None, dataset_toolkit=None):
        ...
        for video in pbar:
            ...
            if dataset_toolkit == 'oneshot':
                self.videos[video] = OTBOneShotVideo(video,
                                              dataset_root,
