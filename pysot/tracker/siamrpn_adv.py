...
class SiamRPNAdv(SiameseTracker):
    def __init__(self, model):
        super(SiamRPNAdv, self).__init__()
    ...
    def init(self, img, bbox, savedir, k=1, attacker=None):
        ...
        with torch.no_grad():
            ...
            if k == -1:
                ...
                savedir = '/media/wattanapongsu/4T/temp/'
                if not os.path.exists(savedir):
                    os.makedirs(savedir)
                self.save(os.path.join(savedir, 'init.jpg'), self.z_crop)
                self.model.template(self.z_crop)
                ...
                if len(self.model.zf) == 1:
                    ...
                    _zf = self.model.zf.squeeze()
                    zfa = _zf/10
                    self.model.zf = zfa.unsqueeze(dim=0)
                else:
                    ...
                    for i in range(3):
                        ...
                        _zf = self.model.zf[i].squeeze()

                        # zfa = _zf[:, torch.randperm(_zf.shape[1])]
                        # zfa = zfa[:, :, torch.randperm(_zf.shape[2])]
                        vals = _zf.shape[0]*_zf.shape[1]*_zf.shape[2]
                        _zfa = _zf.contiguous().view(vals)
                        _zfalike = _zfa.clone()
                        bound = 0.5
                        num = 10
                        _sm = (_zfa > bound).sum() + (_zfa < -bound).sum()
                        while 100*_sm/vals > num:
                            # print('bound + '+ str(bound), end=', ')
                            bound += 0.1
                            _sm = (_zfa > bound).sum() + (_zfa < -bound).sum()
                        sm += _sm

                        _zfalike[_zfa > bound] = -bound
                        _zfalike[_zfa < -bound] = bound
                        # print('mean : ' + str(_zfa.mean().data.cpu().numpy()))
                        # pdb.set_trace()

                        zfa = _zfalike.contiguous().view(_zf.shape[0], _zf.shape[1], _zf.shape[2])
                        self.model.zf[i] = zfa.unsqueeze(dim=0)
                        # pdb.set_trace()
                        # plotheatmap(self.model.zf[i], 'feature')
                    # for i in range(3):
                    #     self.model.zf[i] = 5 + torch.zeros_like(self.model.zf[i]).cuda()
                    print('sum zero = ' + str(100 * (sm.data.cpu().numpy()) / 3 /vals))
