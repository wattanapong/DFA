...
class SiamRPNAttackTemplate(SiameseTracker):
    def __init__(self, model):
        super(SiamRPNAttackTemplate, self).__init__()
        ...
        with torch.no_grad():
            if attacker is not None and k != -99:
                if k != 0:
                    ...
                    self.save(os.path.join(savedir, 'delta.jpg'), 10*zd)
            ...
            if k == -99:
                droprate = 0.1
                self.model.template(self.z_crop, droprate)
                ...
                if len(self.model.zf) == 1:
                    ...
                    dominator = _z2.abs() > droprate
                    print(100 * dominator.sum().data.cpu().numpy() / (256 * 36))
                    ...
                    for i, zf in enumerate(self.model.zf):
                        zf2 = zf / zf.abs().max()
                        dominator = zf2.abs() > droprate
                        print(100 * dominator.sum().data.cpu().numpy() / (256 * 36))
                        # assume the feature can equivalently decrease as 0.2 time
                        zf2[dominator] *= 0.2
                        self.model.zf[i] = zf2
