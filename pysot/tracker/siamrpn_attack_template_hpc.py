...
from tools.frequencyHelper import *
...
class SiamRPNAttackTemplateHPC(SiameseTracker):
    def __init__(self, model):
        super(SiamRPNAttackTemplateHPC, self).__init__()
    ...
    def init(self, img, bbox, savedir, k=1, attacker=None):
        ...
        if attacker is not None:
            ...
            im = self.z_crop.squeeze().permute(1, 2, 0).data.cpu().numpy()

            train_im_l, train_im_h = genFreq(im, 16)
            train_im_h = np.asarray(train_im_h, np.uint8)
            train_im_l = np.asarray(train_im_l, np.uint8)
            np.clip(train_im_l, 0, 255, out=train_im_l)
            np.clip(train_im_h, 0, 255, out=train_im_h)

            plt.figure()
            plt.suptitle('original')
            _im = np.asarray(im, np.uint8)
            plt.imshow(_im[:, :, ::-1])
            plt.figure()
            plt.suptitle('high')
            plt.imshow(train_im_h[:, :, ::-1])
            plt.figure()
            plt.suptitle('low')
            plt.imshow(train_im_l[:, :, ::-1])

            cv2.imwrite('/media/wattanapongsu/4T/temp/save/OTB100/HPC_e19_k1.0/o.jpg', _im)
            cv2.imwrite('/media/wattanapongsu/4T/temp/save/OTB100/HPC_e19_k1.0/h.jpg', train_im_h)
            cv2.imwrite('/media/wattanapongsu/4T/temp/save/OTB100/HPC_e19_k1.0/l.jpg', train_im_l)
            # plt.show()
            pdb.set_trace()

            # z_adv = torch.from_numpy(train_im_h, torch.FloatTensor).permute(2,0,1).unsqueeze(0).cuda()
            z_adv = torch.Tensor(train_im_l).permute(2, 0, 1).unsqueeze(0).cuda()
            z_delta = self.z_crop - z_adv
        mae = 0
        with torch.no_grad():
            if k != 0:
                ...
                # mae = (z_delta[:, :, 1:, 1:] - self.z_crop).abs().mean().data.cpu().numpy()
            ...
            if k == -1:
                if len(self.model.zf) == 1:
                    _z = self.model.zf
                    pdb.set_trace()
                    mean = _z.mean(dim=[2, 3])
                    _z = mean.view(-1, 1).repeat(1, 36).view(1, 256, 6, 6)
                    self.model.zf = _z
                else:
                    _z = torch.stack(self.model.zf)
                    _sd = _z.std(dim=[3, 4])
                    #_zm = _sd * _z.mean(dim=[3, 4])
                    _z = _sd.view(-1, 1).repeat(1, 49).view(3, 1, 256, 7, 7)
                    for i in range(3):
                        self.model.zf[i] = _z[i]
                        # plotheatmap(_z[i], 'feature')
                    # for i in range(3):
                    #     self.model.zf[i] = 5 + torch.zeros_like(self.model.zf[i]).cuda()
